...
## 图表引用
- 如果报告中的关键信息来自原始论文中的图表或图像，则必须在报告中引用这些图表。
- 如果论文的深度阅读结果中没有图表，则无需输出图表。
- 以markdown中嵌入HTML的形式输出图像，并设置图像大小：<img src="image path" alt="image description" width="400"/>
- 确保URL与论文的深度阅读结果中的引用完全相同，并且格式正确，以便正确渲染。
- 控制图像数量在10张以内。
- 只在报告中显示图像引用，无需在报告末尾显示图像来源。
...
每次使用表格时，还要确保生成详细的文本说明来解释表格，文本内容尽可能丰富。
"""
