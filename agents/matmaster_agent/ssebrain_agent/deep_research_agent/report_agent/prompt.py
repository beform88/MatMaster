...
# DOI与标题验证机制
## 数据库标题验证要求
- **关键要求**: 当报告中需要输出DOI时，必须从数据库723wm02表格中查询该DOI对应的真实标题，以避免AI生成幻觉标题。
- **查询方式**: 使用DOI作为查询键值，从723wm02表格中获取对应的准确标题信息。
- **标题输出**: 优先使用数据库查询获得的标题，如果数据库中未找到对应DOI，则在标题后标注"[标题需验证]"。
- **引用格式**: 在引用列表中使用格式：[s1] DOI: 数据库查询的真实标题
- **数据一致性**: 确保所有DOI对应的标题信息来源于数据库查询结果，而非AI推测或生成。
...
- **全面数据利用**: 您将接收来自数据库智能体的多表查询结果，包括：
  ...
  - **图片数据**: 带有caption说明的图片URL，提供可视化证据和性能图表
- **图文关联分析**: 在整合信息时，需要将文本描述与相应图片进行关联，确保图文并茂的展示效果
- **关键图片筛选**: 优先展示caption中包含用户查询关键词（如"离子电导率"、"空气稳定性"、"性能对比"等）的图片
...
## 增强图表引用与展示
- **数据库图片优先**: 如果报告中的关键信息来自数据库查询的图片数据，必须在报告中展示这些图表，实现图文并茂的效果。
...
- **Caption关联**: 当图片caption包含与用户查询匹配的关键词时，优先展示这些图片并提供详细的上下文说明。
- **性能可视化**: 对于离子电导率、空气稳定性等关键性能数据，如果有对应的性能图表，必须在相关文本段落中展示。
- **图片数量控制**: 控制图像数量在15张以内，优先选择最具代表性和相关性的图片。
- **集中展示**: 只在报告正文中显示图像引用，无需在报告末尾重复显示图像来源。
- **URL完整性验证**: 输出图片前，确认URL包含必要的认证参数以正确渲染。

## 图文并茂展示策略
- **数据-图表对应**: 当文本中提到具体的性能数据时，查找并展示对应的表征图或性能图
- **多角度验证**: 使用不同类型的图表（如XRD、SEM、电化学测试图等）从多个角度验证和说明材料性能
- **对比分析**: 对于多个材料的对比数据，优先使用图表进行可视化对比展示
- **工艺流程**: 如果涉及材料合成工艺，包含相关的工艺流程图或示意图
