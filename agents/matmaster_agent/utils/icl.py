...
class ICLExampleSelector:
    def __init__(self, 
        ...
        self.vector_store = FAISS.from_texts(
            [e["update_input"] for e in examples],  
            self.embeddings,
            metadatas=examples
        )
    ...
    def scene_tags_from_examples(self, examples):
        ...
        for example in examples:
            if 'scene_tags' in example:
                scene_prompts.append(f"User Input: {example['update_input']}\nScenes: {', '.join(example['scene_tags'])}\n")
        return "\n".join(scene_prompts)
    ...
    def toolchain_from_examples(self, examples):
        ...
        for example in examples:
            if 'toolchain' in example:
                toolchain_ = " | ".join([f"step{idx+1}: {step}" for idx, step in enumerate(example['toolchain'])])
                toolchain_prompts.append(f"Input: {example['update_input']}\nToolchain: {toolchain_}\n")
    ...
    def expand_input_examples(self, examples):
        ...
        for example in examples:
            if 'update_input' in example:
                expanded_inputs.append(f"Original Input: {example['input']}\nExpanded Input: {example['update_input']}\n")
...
examples = [
    ...
    {"input": "请为我构建一个铁的 bcc 结构", 
     ...
     "toolchain": ['build_bulk_structure_by_template',
                   'optimize_structure'],
    ...
    {"input": "对比不同数据库中 Fe₂O₃ 的带隙数据",
     ...
     "toolchain": ["fetch_structures_with_bandgap",
    ...
    {"input": "帮我建一个H₂O在 TiO₂ 表面 (101) 吸附的结构",
     ...
     "toolchain": ["fetch_structures_with_filter",
                   "build_surface_slab",
                   "make_supercell_structure",
    ...
    {"input": "生成氮气和氧气的混合气体盒子，比例为2:1，盒子大小为15×15×15 Å³",
     ...
     "toolchain": ["build_molecule_structures_from_smiles",
                   "build_molecule_structures_from_smiles",
