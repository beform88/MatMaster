...
async def document_parser_before_tool(tool, args, tool_context: ToolContext):
    """
    before_tool_callback to validate tool arguments before execution
    """
    tool_info = {
        'before': {
            'tool_name': tool.name,
...
async def document_parser_after_tool(tool, args, tool_context: ToolContext, tool_response: CallToolResult):
    """
    after_tool_callback to track tool execution results
    """
    tool_info = {
        'after': {
            'tool_name': tool.name,
...
def _update_tool_context(tool_context, tool_info):
    """Helper function to update tool context state"""
    if 'document_parser_tool_calls' not in tool_context.state:
        tool_context.state['document_parser_tool_calls'] = []

    tool_context.state['document_parser_tool_calls'].append(tool_info)
