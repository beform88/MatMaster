from dotenv import load_dotenv
from dp.agent.adapter.adk import CalculationMCPToolset
from google.adk.agents import BaseAgent
from google.adk.tools.mcp_tool.mcp_session_manager import SseServerParams

from agents.matmaster_agent.constant import (
    LOCAL_EXECUTOR,
    MATMASTER_AGENT_NAME,
    BohriumStorge,
)
from agents.matmaster_agent.core_agents.public_agents.sync_agent import (
    BaseSyncAgentWithToolValidator,
)
from agents.matmaster_agent.llm_config import LLMConfig
from agents.matmaster_agent.sub_agents.STEELkb_agent.constant import (
    STEEL_SERVER_URL,
)
from agents.matmaster_agent.sub_agents.STEELkb_agent.prompt import STEELKbAgentName

load_dotenv()

# Initialize MCP tools for STEELkb Knowledge Base
steel_kb_toolset = CalculationMCPToolset(
    connection_params=SseServerParams(url=STEEL_SERVER_URL),
    storage=BohriumStorge,
    executor=LOCAL_EXECUTOR,
)


class STEELKbAgent(BaseSyncAgentWithToolValidator):
    def __init__(self, llm_config: LLMConfig, name_suffix=''):
        super().__init__(
            model=llm_config.default_litellm_model,
            doc_summary=True,
            name=STEELKbAgentName + name_suffix,
            description='',
            instruction='',
            tools=[steel_kb_toolset],
            render_tool_response=False,
            supervisor_agent=MATMASTER_AGENT_NAME,
        )


def init_steel_kb_agent(llm_config: LLMConfig, name_suffix='') -> BaseAgent:
    return STEELKbAgent(llm_config, name_suffix)
