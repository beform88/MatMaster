...
async def after_tool_callback(tool, args, tool_context, tool_response):
    ...
    for paper_info in output_paper_list:
        ...
        if 'Abstract' in new_info and isinstance(new_info['Abstract'], str) and len(new_info['Abstract']) > MAX_ABSTRACT_LEN:
    ...
    # write in a log file
    with open('filtered_paper_info.log', 'a') as f:
        f.write(f"[after_tool_callback] Filtered paper info, length: {len(new_content)}")
