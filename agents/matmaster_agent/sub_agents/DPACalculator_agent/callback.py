...
BUILTIN_DPA2_HEADS = ['Domains_Alloy', 'Domains_Anode', 'Domains_Cluster', 
...
'OC22', 'Organic_Reactions', 'RANDOM']
BUILTIN_DPA3_HEADS = ['Domains_Alloy', 'Domains_Anode', 'Domains_Cluster',
'Domains_Drug', 'Domains_FerroEle', 'Domains_SSE_PBE', 'Domains_SemiCond', 'H2O_H2O_PD', 
...
async def before_tool_callback(tool, args, tool_context: ToolContext):
    ...
    if model_path and "https://bohrium.oss-cn-zhangjiakou.aliyuncs.com/13756/27666/store/upload/cd12300a-d3e6-4de9-9783-dd9899376cae/dpa-2.4-7M.pt" in model_path:
        if head not in BUILTIN_DPA2_HEADS:
            args['head'] = 'Omat24'
            print(f"[before_tool_callback] Updated head to Omat24 for DPA2.4 model {head}, {args['head']}")
            from time import sleep; sleep(33)
    elif model_path and "https://bohrium.oss-cn-zhangjiakou.aliyuncs.com/13756/27666/store/upload/18b8f35e-69f5-47de-92ef-af8ef2c13f54/DPA-3.1-3M.pt" in model_path:
        if head not in BUILTIN_DPA3_HEADS:
            args['head'] = 'Omat24'
            print(f"[before_tool_callback] Updated head to Omat24 for DPA3.1 model {head}, {args['head']}")
