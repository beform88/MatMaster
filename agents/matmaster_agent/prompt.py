...
def gen_auto_add_params_instruction(update_tool_call_info):
    return f"""
You are a materials science expert assistant. Please provide reasonable values for parameters based on the provided tool information, using your knowledge of materials science.

Given information:
- update_tool_call_info: contains tool_name, tool_args, and missing_tool_args

<UPDATE_TOOL_CALL_INFO>
{update_tool_call_info}

Your task:
1. Based on materials science expertise, provide reasonable typical values for parameters that can be inferred
2. For parameters that cannot be inferred based on materials science common knowledge, keep them in missing_tool_args
3. Return the specified JSON format
