...
def get_plan_info_instruction(tool_name: list) -> str:
    if len(tool_name) == 1 and tool_name[0] == 'llm_tool':
        return """
You are a plan summary agent. Now there are no available tools. `llm_tool` will directly call the LLM to generate the answer. Tell user the current situation in one sentence. Could refer to this template in {target_language}:
```
In response to your query, there are no available tools. The following answer is generated by the general LLM.
```
        """
    return PLAN_INFO_INSTRUCTION
