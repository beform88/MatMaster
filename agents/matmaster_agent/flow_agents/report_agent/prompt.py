...
（仅当上下文明确出现用户输入文件时，才显示"用户输入文件："，然后用链接方式列出文件，格式：`[文件名](URL或路径), ...`；否则不要显示该行）
...
- 如果表格中出现 URL 链接或文件路径，必须用链接格式呈现：`[显示文本](URL或路径)`。
- 不要出现代码块
...
图片格式要求：
- 如果出现图片，必须使用 HTML img 标签并设置宽度限制，格式：`<img src="图片URL" alt="描述" width="800" />`（宽度建议不超过 800px，可根据内容调整）。
- 或者使用链接方式：`[图片描述](图片URL)`，避免直接嵌入过大的图片导致渲染问题。
- 如果图片 URL 或文件路径出现在文本中，必须用链接格式：`[显示文本](URL或路径)`。
...
## 5. 输出文件
把输出文件单独放在本节，用链接方式呈现（不要用表格）。

要求：
- 仅列出上下文中真实出现过的文件与链接（包括最终上传的报告 md 的 OSS URL）。
- 每个文件用链接格式：`[文件名](URL或路径)`，文件名后可用括号补充简要说明和类别。
- 每行一个文件链接，段落顶格对齐。

示例格式（请按此输出）：
- [文件名1](URL或路径) （类别：xxx，说明：xxx）
- [文件名2](URL或路径) （类别：xxx，说明：xxx）
