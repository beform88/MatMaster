...
from agents.matmaster_agent.chembrain_agent.constant import (
    LOADING_DESC,
    LOADING_START,
    LOADING_STATE_KEY,
    LOADING_TITLE,
...
def paper_list_before_agent(callback_context: CallbackContext):
    ...
    try:
        ...
        while toolcall_infos and (
            toolcall_infos[-1]['tool_name'] != 'query_table'
            or toolcall_infos[-1]['tool_response'].get('paper_count', 0) == 0
        ):
            toolcall_infos.pop()
...
class GroupPaperAgent(BaseAgent):
    ...
    async def _run_async_impl(self, ctx):
        ...
        loading_desc_msg = 'Please wait while we extract key insights... Your report will be generated shortly.'
        ...
        _ = ParallelAgent(
            ...
            sub_agents=[
                init_paper_agent(MatMasterLlmConfig, name=n, run_id=run_id)
                for n in POOL
            ],
        )
