...
from agents.matmaster_agent.chembrain_agent.smiles_conversion_agent.constant import SMILESConversionToolCall
...
async def smiles_conversion_after_tool(tool, args, tool_context: ToolContext, tool_response: CallToolResult):
    ...
    if not (tool_response and
            ...
            json.loads(tool_response.content[0].text).get('output_path', None) is not None):
        _update_tool_context(tool_context, tool_info)
        return None

    try:
        markdown_image = f"![mol_png]({json.loads(tool_response.content[0].text)['output_path']})"
